...
def evaluate_dataset(pipe, data, dataset_name, verbose=False, n_generate_sample=6, n_select_sample=3, depth=3):
    ...
    # save to pickle
    with open(f"{output_dir}/results_tot_train.pkl", "wb") as f:
        pickle.dump(results, f)
    
    # save to json
    with open(f"{output_dir}/results_tot_train.json", "w") as f:
...
datasets = {
    "VitaminC": f"{data_dir}/VitaminC/train_truncated_300.jsonl",
    "2WikiMultihopQA": f"{data_dir}/2WikiMultihopQA/train_truncated_300.json",
    "Bamboogle": f"{data_dir}/Bamboogle/train_truncated_300.json",
    "FEVER": f"{data_dir}/FEVER/fever_train_truncated_300.jsonl",
    "FEVEROUS": f"{data_dir}/FEVEROUS/feverous_train_truncated_300.jsonl",
    "HotpotQA": f"{data_dir}/HotpotQA/train_truncated_300.json",
    "SVAMP": f"{data_dir}/SVAMP/train.json"
}
...
for dataset_name in datasets_to_evaluate:
    ...
    data = load_dataset(file_path, dataset_name, limit=300)  # Limit to 150 samples
